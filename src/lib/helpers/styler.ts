...
export function createDraggedElementFrom(originalElement, positionCenterOnXY) {
	...
	draggedEl.style.transition = `${trs('top')}, ${trs('left')}, ${trs('background-color')}, ${trs(
		'opacity'
	)}, ${trs('color')} `;
...
export function morphDraggedElementToBeLike(
	...
	if (widthChange || heightChange) {
		...
		draggedEl.style.left = `${
			parseFloat(draggedEl.style.left) -
			relativeDistanceOfMousePointerFromDraggedSides.left * widthChange
		}px`;
		draggedEl.style.top = `${
			parseFloat(draggedEl.style.top) -
			relativeDistanceOfMousePointerFromDraggedSides.top * heightChange
		}px`;
	}
...
function copyStylesFromTo(copyFromEl, copyToEl) {
	...
	Array.from(computedStyle)
		...
		.forEach((s) =>
			copyToEl.style.setProperty(
				s,
				computedStyle.getPropertyValue(s),
				computedStyle.getPropertyPriority(s)
			)
		);
}
